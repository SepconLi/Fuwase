...
CREATE TABLE [dbo].[tmp_ms_xx_Notification] (
    ...
    CONSTRAINT [tmp_ms_xx_constraint_PK_Notification1] PRIMARY KEY CLUSTERED ([Email] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[Notification])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_Notification] ([Email], [Id], [NotificationText], [Link], [ReadNotification], [TimeNotification])
        SELECT   [Email],
                 [Id],
                 [NotificationText],
                 [Link],
...
GO
PRINT N'Altering Table [dbo].[Statistic]...';


GO
ALTER TABLE [dbo].[Statistic]
    ADD [App_Total_Donations] INT DEFAULT 0 NULL;


GO
PRINT N'Creating Foreign Key [dbo].[PK_Notification_To_User]...';


GO
ALTER TABLE [dbo].[Notification] WITH NOCHECK
    ADD CONSTRAINT [PK_Notification_To_User] FOREIGN KEY ([Email]) REFERENCES [dbo].[_User] ([Email]);


GO
PRINT N'Altering Trigger [dbo].[top_businesses_update]...';


GO
ALTER TRIGGER top_businesses_update
ON dbo.statistic
AFTER Update
AS
BEGIN
...
END
GO
PRINT N'Altering Procedure [dbo].[get_total_app_donations]...';


GO
ALTER PROC get_total_app_donations
AS
SELECT count(id)  AS  'App_Total_Donations', '' AS 'User_Id',
101.5E5 AS 'Donated_Weight', 0 AS 'Order_Amount', 0 AS 'Donation_Amount'
FROM Donation
GO
PRINT N'Altering Procedure [dbo].[GetCampaignDonationAmount]...';


GO
ALTER PROC GetCampaignDonationAmount
    @campaignId INT
AS
BEGIN
    SELECT COUNT(Id)
...
END;
GO
PRINT N'Altering Trigger [dbo].[statistics_update]...';


GO
ALTER TRIGGER statistics_update
ON dbo.donation
AFTER Update
AS
...
GO
PRINT N'Refreshing Procedure [dbo].[get_donations_made]...';


GO
EXECUTE sp_refreshsqlmodule N'[dbo].[get_donations_made]';


GO
PRINT N'Refreshing Procedure [dbo].[get_orders_made]...';


GO
EXECUTE sp_refreshsqlmodule N'[dbo].[get_orders_made]';
...
GO
ALTER TABLE [dbo].[Notification] WITH CHECK CHECK CONSTRAINT [PK_Notification_To_User];
